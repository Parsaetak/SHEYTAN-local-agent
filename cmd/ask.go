...
func Ask(cfg *config.Config, args []string) int {
	...
	multi := fs.Bool("multi", false, "use the planner→executor→critic multi-agent pipeline")
	newSess := fs.Bool("new", false, "start a fresh session instead of reusing the latest")
	noStart := fs.Bool("no-llm-start", false, "do not auto-start the local llama.cpp server")
	sessID := fs.String("session", "", "continue a specific session id")
	think := fs.Bool("think", false, "enable thinking mode for this turn (step-by-step reasoning)")
	toolSel := fs.String("tools", "", "comma-separated tool allow-list (e.g. files,shell,dataAnalysis); empty = all")
	attach := fs.String("attach", "", "comma-separated file paths to attach to the message (text files inline, binaries as metadata)")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	prompt := strings.Join(fs.Args(), " ")
	if prompt == "" && *attach == "" {
		fmt.Fprintln(os.Stderr, `usage: sheytan ask [--multi] [--new] [--no-llm-start] [--session ID] [--think] [--tools a,b] [--attach f1.txt,f2.md] "your prompt"`)
		return 2
	}
	...
	if *toolSel != "" {
		var list []string
		for _, t := range strings.Split(*toolSel, ",") {
	...
	composed, images := chunking.ComposeWithImages(prompt, attached, cfg.AttachmentsBudgetBytes())
	...
	if !*noStart {
		if err := stack.EnsureLLM(); err != nil {
			fmt.Fprintf(os.Stderr, "LLM backend: %v\n", err)
			return 1
		}
	}
	...
	if sess.Context.SystemPrompt != "" {
		msgs = append(msgs, llm.Message{Role: "system", Content: sess.Context.SystemPrompt})
	}
	msgs = append(msgs, sess.Messages...)
	now := time.Now()
	msgs = append(msgs, llm.Message{Role: "user", Content: composed, Attachments: attachNames, Images: images, At: now})
	sess.Messages = append(sess.Messages, llm.Message{Role: "user", Content: composed, Attachments: attachNames, Images: images, At: now})
	if len(sess.Messages) == 1 {
		...
		sess.Title = title
		_ = store.UpdateTitle(sess.ID, title)
	}
	_ = store.Save(sess)

	fmt.Printf("┌─ SHEYTAN™ v%s — ask (provider=%s, model=%s)\n", config.AppVersion, cfg.ProviderKind(), cfg.EffectiveModel())
	fmt.Printf("│ session: %s\n", sess.ID)
	if len(attachNames) > 0 {
		fmt.Printf("│ attach: %s\n", strings.Join(attachNames, ", "))
	}
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				...
				logging.Default().Crash(r, buf[:n])
				runErr = fmt.Errorf("panic: %v", r)
			}
		}()
		var err error
		if *multi {
			final, err = stack.Multi.Run(ctx, prompt, printActivity)
		} else {
			var res agent.RunResult
			res, err = stack.Orch.RunDetailed(ctx, msgs, printActivity)
			final = res.Text
	...
	if final != "" {
		sess.Messages = append(sess.Messages, llm.Message{Role: "assistant", Content: final, Reasoning: reasoningTrace})
		_ = store.Save(sess)
		if reasoningTrace != "" {
			fmt.Printf("\n%s\n  (thinking trace: %d chars — see the session file)\n", strings.Repeat("─", 60), len(reasoningTrace))
		}
		fmt.Printf("\n%s\n", strings.Repeat("─", 60))
		fmt.Println(final)
	}
	logging.Default().Info("ask", "turn complete (session=%s, reply=%d chars)", sess.ID, len(final))
	// v1.0.2: index the completed exchange into persistent recall.
	if stack.Recall != nil && final != "" {
		_ = stack.Recall.IndexTurn(sess.ID, sess.Title, prompt, final, nil)
	}
	return 0
}
