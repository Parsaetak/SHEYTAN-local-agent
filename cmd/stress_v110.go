...
// decoded, case preserved), size cap enforced, non-http rejected. The
// fetch tool's SSRF guard is relaxed for the loopback httptest server —
// scheme, credential, and file:// rules stay fully enforced.
func stressV110FetchText() error {
        prev := tools.SetFetchPrivateDestinationsAllowedForTest(true)
        defer tools.SetFetchPrivateDestinationsAllowedForTest(prev)
