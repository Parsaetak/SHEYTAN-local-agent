...
// Abort signals the in-flight run to stop at the next iteration boundary.
...
func (o *Orchestrator) RunDetailed(ctx context.Context, messages []llm.Message, onActivity func(Activity)) (RunResult, error) {
        ...
        for iter := 0; iter < maxIter; iter++ {
                if o.isAborted() {
                        onActivity(Activity{Type: "done", Caption: "Aborted by user", Timestamp: time.Now()})
                        return result, nil
                }
                ...
                if len(lastToolCalls) == 0 {
                        ...
                        onActivity(Activity{Type: "done", Caption: "Completed", Timestamp: time.Now()})
                        return result, nil
                }
                ...
                for _, tc := range lastToolCalls {
                        ...
                        if ok && !o.cfg.ToolEnabled(tc.Function.Name) {
                                ...
                                // of retrying.
                                ok = false
                                result2 = fmt.Sprintf("Error: tool %q is disabled by the user. Enabled tools: %s. Do not call disabled tools — adapt your plan.",
                                        tc.Function.Name, strings.Join(names, ", "))
                        }
                        if !ok && result2 == "" {
                                result2 = fmt.Sprintf("Error: unknown tool %q. Available: %s", tc.Function.Name, strings.Join(names, ", "))
                        }
                        ...
                        // the path rides the tool message's Images field and
                        ...
                        if err != nil {
                                ...
                                if result2 != "" {
                                        result2 = fmt.Sprintf("Error: %v\n\nTool output:\n%s", err, truncate(result2, 4000))
                                } else {
        ...
        result.ToolsUsed = toolList(toolsUsed)
        result.ContextUsage = peakUsage
        onActivity(Activity{Type: "done", Caption: "Max iterations reached", Timestamp: time.Now()})
