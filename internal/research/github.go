...
//   - known workarounds
func (p *GitHubProvider) Search(ctx context.Context, req SearchRequest) (SearchResponse, error) {
	req = req.Normalize()
	...
	endpoint, err := url.Parse(p.BaseURL + "/search/issues")
	if err != nil {
		return SearchResponse{}, fmt.Errorf("%w: invalid GitHub endpoint: %v", ErrProviderUnavailable, err)
	}
	...
	if err != nil {
		return SearchResponse{}, fmt.Errorf("create GitHub request: %w", err)
	}

	request.Header.Set("Accept", "application/vnd.github+json")
	request.Header.Set("User-Agent", "SHEYTAN-Local-Agent/Version-Zeta")
	...
	for _, item := range payload.Items {
		...
		publishedAt := parseGitHubTime(item.CreatedAt)
		...
		snippet := compactGitHubBody(item.Body, 700)

		matchScore := item.Score
		if matchScore > 1 {
			// GitHub's search score is not normalized to [0,1].
			// Convert it conservatively for our common result model.
			matchScore = 1
		}
		if matchScore < 0 {
			matchScore = 0
...
func compactGitHubBody(body string, maxLen int) string {
...
func githubHTTPError(response *http.Response) error {
