// Package native — v1.0.8 low-level Windows system access.
//
// THE ATTACHMENT-CRASH FIX (v1.0.8): the app previously opened attachments
// through Fyne's built-in file dialog, which walks the filesystem in its
// own goroutines to list folders. On real Windows machines that walker can
...
// time (the Windows build already cross-compiles with mingw for GLFW, but
// these files must stay pure so they also work in CGO_ENABLED=0 test
// builds).
package native
