...
// the future heavy-compute/AI execution engine. In Phase 1 the native
// engine implements the supervised lifecycle (start / health-check / mark
// ready / stop / detect failure / bounded restart), hardware reporting and
// metrics — NOT inference. Generation requests still run on the llama.cpp
// backend, which remains fully functional as the fallback.
//
...
//     granularity.
//
// # Concern layout (Phase 1)
//
// The package separates the future engine's concerns as files:
//
//	protocol.go   wire protocol (framing, ops, validation)
...
//	model.go      model lifecycle types (types only — Phase 1)
//	memory.go     memory plan types (types only — Phase 1)
//	kv.go         KV-cache types (types only — Phase 1)
//	generation.go generation request types (types only — Phase 1)
//	scheduler.go  request scheduler types (types only — Phase 1)
//
