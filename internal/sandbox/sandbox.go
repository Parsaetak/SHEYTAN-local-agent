...
func NewCodeExecSandbox(memMB, cpuPct int, dir string) (*CodeExecSandbox, error) {
        tempDir := false
        if dir == "" {
        ...
        sb := &CodeExecSandbox{memMB: memMB, cpuPct: cpuPct, dir: dir, tempDir: tempDir}
        // Best-effort governor: nil means timeout-only execution.
        sb.gov = newGovernor(memMB, cpuPct)
        return sb, nil
}

// WorkDir returns the sandbox workdir (scripts run there).
func (s *CodeExecSandbox) WorkDir() string { return s.dir }

// MemMB / CPUPct expose the configured caps.
func (s *CodeExecSandbox) MemMB() int  { return s.memMB }
func (s *CodeExecSandbox) CPUPct() int { return s.cpuPct }

// --- agent.Tool surface ---------------------------------------------------

// Name implements the agent tool interface (replaces plain codeExec).
func (s *CodeExecSandbox) Name() string { return "codeExec" }

// Description implements the agent tool interface.
func (s *CodeExecSandbox) Description() string {
        return "Run code inside the resource-capped sandbox (memory " + fmt.Sprintf("%d MB", s.memMB) +
                ", CPU " + fmt.Sprintf("%d%%", s.cpuPct) + ") and return stdout+stderr. " +
                "lang: python (default) or node. Code runs in the sandbox workdir — files it writes stay there for the files/dataAnalysis tools."
}
...
func (s *CodeExecSandbox) Run(ctx context.Context, args json.RawMessage) (string, error) {
        var p struct {
                Lang    string `json:"lang"`
                Code    string `json:"code"`
        ...
        out, err := s.Execute(ctx, bin, script)
...
func (s *CodeExecSandbox) stageScript(ext, code string) (string, func(), error) {
        s.mu.Lock()
        if s.closed {
                s.mu.Unlock()
        ...
        seq := s.seq
        s.mu.Unlock()
        script := filepath.Join(s.dir, fmt.Sprintf("run-%04d%s", seq, ext))
...
func (s *CodeExecSandbox) Execute(ctx context.Context, name string, args ...string) ([]byte, error) {
        s.mu.Lock()
        closed := s.closed
        dir := s.dir
        gov := s.gov
        ...
        if gov != nil {
                name, args = gov.prepare(timeoutSec, name, args)
        }
        ...
        if gov != nil {
                _ = gov.attach(cmd.Process) // best-effort (Windows job assignment)
        }
        waitErr := cmd.Wait()
...
// pythonBin resolves the Python interpreter across platforms (python3 on
// Unix, python/py on Windows) — mirrors the plain codeExec tool so the
// sandbox is a true drop-in.
