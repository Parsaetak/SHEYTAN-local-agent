#!/usr/bin/env bash
# Build + package SHEYTAN-Local-Agent v1.1.0 (Zeta) — Windows native desktop app
...
#
# v1.1.0 (Zeta) — the Node-UI release:
#   - The legacy Fyne desktop UI (internal/ui + fyne.io/* dependencies) is
#     FULLY REMOVED. The app now runs the React/Vite frontend embedded via
#     web/static and served by the Wails v3 asset layer (internal/desktop).
#   - scripts/gen-syso no longer needs the fyne-io SVG rasterization stack:
#     the brand flame ships as a committed 512px PNG (go:embed).
#   - The Windows exe cross-compiles with CGO_ENABLED=0 — Wails v3 on
#     Windows is cgo-free, so no mingw/X11 header environment is required.
